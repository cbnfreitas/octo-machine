...
from app.narrator_prompt.section_tools_move_and_dice import tools_move_and_dice_section


def build_rpg_sections(config: NarratorPromptConfig) -> str:
    parts: list[str] = []
    if config.include_fixed_intro_context:
        parts.append(fixed_intro_context_section())
    if config.include_acrobatics_fatigue_time:
        parts.append(acrobatics_fatigue_time_section())
    if config.include_role_world_rules:
        parts.append(role_world_rules_section())
    if config.include_tools_move_and_dice:
        parts.append(tools_move_and_dice_section(config))
