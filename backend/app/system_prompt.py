...
def _rpg_sections() -> str:
    return (
        ...
        "oposição): envie **`skill`** (texto livre que descreve a manobra; ainda não muda o sorteio) e "
        "**`difficulty`** (`muito_facil`, `facil`, `medio`, `dificil`, `muito_dificil`) coerente com a "
        "dificuldade que já estabeleceste na cena. A ferramenta devolve falha crítica, falha, sucesso ou "
        "sucesso crítico com as probabilidades desse nível. Para números em intervalo ou cara/coroa "
        "isolados, use as outras ferramentas. Integre qualquer resultado ao que você narrar em seguida.\n\n"
