...
def _rpg_sections() -> str:
    ...
    return (
        ...
        "**`description`** é a impressão principal (prioridade na **primeira** narração ao chegar); "
        "**`details`** é a segunda camada (textura extra, arrumação, micro-cena) que também recebes no "
        "JSON—usa-a quando o jogador **aprofundar**, pedir mais, ou quando a ação o exigir, **sem** "
        "despejar tudo na chegada. O **`description_full`** junta as duas camadas já filtradas; só "
        "invoca trechos ainda omitidos quando a descoberta for merecida. **Não invente absolutamente nenhum fato** sobre o "
        ...
        "**Segredos e descoberta:** `move` já devolve **`description`**, **`details`** e **`player_facing_summary` "
        "sem os blocos marcados como segredo/armadilha no arquivo—trate isso como o que o jogador "
        "**pode notar** conforme a camada; reescreva com sua voz, sem colar texto técnico. O **`description_full`** "
        "junta as duas camadas filtradas; **nunca** entregue segredo, mecanismo oculto, chave ou tesouro **do nada**: "
        ...
        "Ao narrar a **chegada** (primeira impressão do lugar), baseie-se sobretudo em **`description`** "
        "e em `player_facing_summary` **e** nas **conexões** retornadas (objetos com `to`, `how` e "
        "sinalizadores de passagem). Há **`details`** no mesmo JSON para aprofundar quando fizer sentido; "
        "em prosa, priorize o "
        ...
        "você pode aprofundar com base no que o `move` / mapa sustenta, incluindo a camada **`details`** "
        "quando o foco do jogador o justificar. **Depois** que o lugar já foi "
