...
def _rpg_sections() -> str:
    ...
    return (
        ...
        "janelas, portas e iluminação deve vir **exclusivamente** das camadas devolvidas por **`move`**: "
        "**`basic_description`** é a impressão principal e é a **única** camada de sítio presente no "
        "JSON quando **`revisit`: false** (chegada inicial àquele lugar nesta sessão); **não** infiras "
        "nem inventes o que estaria só em **`details`**, que o motor **retém** até **`revisit`: true**. "
        "Com **`revisit`: true**, o JSON inclui **`details`** (textura extra); aí podes integrar o que "
        "fizer sentido. O **`description_full`** na primeira visita coincide com `basic_description`; "
        "em revisit junta as duas camadas filtradas. Só invoca trechos ainda omitidos quando a descoberta "
        "for merecida. **Não invente absolutamente nenhum fato** sobre o "
        ...
        "sempre filtrados; o campo **`details`** só aparece no JSON quando **`revisit`: true**. "
        ...
        "**Camada `details` vs. trecho de segredo no mapa:** em **`details`**, o texto **antes** de "
        ...
        "usa o trecho de **`details` antes dos marcadores de segredo** e **não** exige dado. Se o jogador "
        "fizer busca **ampla, vaga ou indireta** (ex.: \"vasculho tudo\", \"tem mais alguma coisa na "
        "mesa?\", \"tem algo escondido?\", \"procuro no cômodo\"), trate como percepção/investigação "
        "incerta: **chame `roll_dice`** para decidir **só** o que estiver nos **trechos de segredo** "
        "aplicáveis; o perceptível não secreto **já** podes narrar no mesmo turno **antes** ou **junto** "
        "com o resultado, sem inventar além do mapa.\n\n"
        ...
        "você pode aprofundar com base no que o `move` / mapa sustenta; com **`revisit`: true** usa também "
        "**`details`** se estiver no JSON. **Depois** que o lugar já foi "
